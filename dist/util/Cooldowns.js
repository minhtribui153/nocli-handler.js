...
class Cooldowns {
    ...
    _dbRequired;
    constructor(options) {
        const { instance, errorMessage = "Please wait [TIME] before trying again", botOwnersBypass = false, dbRequired = 300 // 5 minutes
         } = options;
    ...
    async loadCooldowns() {
        ...
        const cooldowns = await cooldown_schema_1.default.find({});
        for (const cooldown of cooldowns) {
            const { _id, expires } = cooldown;
            this._cooldowns.set(_id, expires);
        }
    }
    getKeyFromCooldownUsage(options) {
    ...
    verifyCooldown(duration) {
        ...
        if (split.length !== 2) {
            throw new NoCliCommandError_1.default(`Duration "${duration}" is an invalid duration. Please use "10 m", "15 s" etc`);
        }
        const quantity = +split[0];
        const type = split[1].toLowerCase();
        if (!cooldownDurations[type])
            throw new NoCliCommandError_1.default(`Unknown duration type "${type}". Please use one of the following: ${Object.keys(cooldownDurations).join(', ')}`);
        if (quantity <= 0)
            throw new NoCliCommandError_1.default(`Invalid quantity of "${quantity}". Please use a value greater than 0.`);
        // @ts-ignore
        return quantity * cooldownDurations[type];
    }
    /** Gets the cooldown key */
    getKey(cooldownType, userId, actionId, guildId) {
        const isPerUser = cooldownType === types_1.cooldownTypesArray[0];
        const isPerUserPerGuild = cooldownType === types_1.cooldownTypesArray[1];
        const isPerGuild = cooldownType === types_1.cooldownTypesArray[2];
        const isGlobal = cooldownType === types_1.cooldownTypesArray[3];
        if ((isPerUserPerGuild || isPerGuild) && !guildId)
            throw new NoCliCommandError_1.default(`Invalid cooldown type "${cooldownType}" used outside of a guild`);
        if (isPerUser)
    ...
    /** Starts the cooldown */
    async start(options) {
        const { cooldownType, userId, actionId, guildId = '', duration } = options;
        const cooldownDuration = duration;
        if (this.canBypass(userId))
            return;
        if (!types_1.cooldownTypesArray.includes(cooldownType))
            throw new NoCliCommandError_1.default(`Invalid cooldown type "${cooldownType}". Please use one of the following: ${types_1.cooldownTypesArray.join(', ')}`);
    ...
    canRunAction(options) {
        const { cooldownType, userId, actionId, guildId = '', errorMessage = this._errorMessage } = options;
        if (this.canBypass(userId))
            return true;
        const key = this.getKey(cooldownType, userId, actionId, guildId);
