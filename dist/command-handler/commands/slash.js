...
exports.default = {
    ...
    autocomplete: async (interaction, command, arg) => {
        if (arg === "target") {
            ...
            if (target !== "Global" && target !== "Current Guild") {
                ...
                if (application)
                    application.commands.cache.forEach((cmd, key) => allCommands ? allCommands.cache.set(key, cmd) : "");
            }
            const allCommandsKeyIds = [];
            if (allCommands)
                allCommands.cache.forEach(command => allCommandsKeyIds.push(`${command.name}-${command.id}`));
    ...
    callback: async ({ interaction, options, instance, guild, args }) => {
        ...
        const target = options.getString('target', true);
        // commandName-commandId
        ...
        if (!commandKeyId) {
            ...
            if (!allCommandsKeyIds.length)
                embed.setFooter({ text: "No Commands Found" });
            return {
                embeds: [embed],
                ephemeral: true
            };
